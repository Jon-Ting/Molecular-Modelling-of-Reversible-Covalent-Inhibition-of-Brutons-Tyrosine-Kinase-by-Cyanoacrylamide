...
if __name__ == "__main__":
    ...
    fig.suptitle("Cluster Populations for All Inhibitors", horizontalalignment='center', fontsize=14, weight='bold')
    ...
    for i, inhibitor in enumerate(inhibitor_list):
        ...
        label_list = []
        for j in [1, 2]:
            DATA_DIR = "{0}/{1}/MD/{2}/Rep{3}/analysis/cluster".format(MD_PATH, str(inhibitor), system_type, j)
            for k in ["A", "B"]:
    ...
    plt.savefig("{0}/Cluster Populations for All Inhibitors {1}".format(CURR_DIR, fig_type))
    plt.show()
